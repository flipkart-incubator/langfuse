...
const NewOxfordViewBaseSchema = z.object({
...
const NewChatOxfordViewSchema = NewOxfordViewBaseSchema.extend({
...
const NewTextOxfordViewSchema = NewOxfordViewBaseSchema.extend({
  type: z.literal(PromptType.Text),
  chatPrompt: z.array(z.any()),
  textPrompt: TextPromptContentSchema,
});

export const NewOxfordViewFormSchema = z.discriminatedUnion("type", [
  NewChatOxfordViewSchema,
  NewTextOxfordViewSchema,
]);
export type NewOxfordViewFormSchemaType = z.infer<
  typeof NewOxfordViewFormSchema
>;

export const OxfordViewVariantSchema = z.union([
  z.object({
    type: z.literal(PromptType.Chat),
    prompt: PromptChatMessageListSchema,
  }),
  z.object({ type: z.literal(PromptType.Text), prompt: z.string() }),
]);
export type OxfordViewVariant = z.infer<typeof OxfordViewVariantSchema>;

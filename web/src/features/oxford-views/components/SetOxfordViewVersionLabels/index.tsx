...
export function SetOxfordViewVersionLabels({
  promptLabels,
  prompt,
  isOpen,
  setIsOpen,
  ...
  const usedLabelsInProject = api.oxfordViews.allLabels.useQuery(
  ...
  const mutateLabels = api.oxfordViews.setLabels.useMutation({
    onSuccess: () => void utils.oxfordViews.invalidate(),
  });

  const handleSubmitLabels = async () => {
    try {
      if (!projectId) return;
      await mutateLabels.mutateAsync({
        projectId: projectId as string,
        promptId: prompt.id,
  ...
  return (
    <Popover open={isOpen} onOpenChange={handleOnOpenChange} modal={false}>
      ...
      >
        ...
        >
          <h2 className="text-md mb-3 font-semibold">Oxford View labels</h2>
          <h2 className="mb-3 text-xs">
            Use labels to identify different versions. The{" "}
            <strong>production</strong> labeled view will be served by default.
          </h2>
          ...
          <Button
            ...
            loading={mutateLabels.isPending}
